...
or a custom authentication UI using the [ReactJS](https://reactjs.org/) framework
...
provided using the [Angular framework](../authui/README.md).
...
```bash
cd iap-gcip-web-toolkit/sample/authui-react
npm install
```
...
- **Using FirebaseUI**
  ...
  required `tenantsConfig` object in `src/components/firebaseui.tsx`.
...
- **Using a custom UI**
  ...
  `SAML_PROVIDER_ID` constant in `src/components/app.tsx`
...
- The `AppRouter` need to be modified in `src/components/root.tsx` to
...
by running `npm run start` in the same directory `sample/authui-react`,
