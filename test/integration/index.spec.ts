...
describe('GCIP/IAP sign-in automated testing', () => {
  ...
  // for IAP resource configured with one GCIP tenant.
});
 ...
 */
function cleanup(app: admin.app.App, uids: string[]): Promise<void[]> {
  const promises: Array<Promise<void>> = [];
  uids.forEach((uid) => {
    // Use safeDelete to avoid getting throttled.
    promises.push(safeDelete(app, uid));
  });
 ...
 */
function safeDelete(app: admin.app.App, uid: string): Promise<void> {
